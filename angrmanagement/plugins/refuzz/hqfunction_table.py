...
class QHFunctionTableModel(QAbstractTableModel):
    ...
    def __init__(self, workspace, func_list, plugin):

        super(QHFunctionTableModel, self).__init__()
    ...
    def headerData(self, section, orientation, role):
        ...
        if section < len(self.Headers):
            ...
            try:
                return self.workspace.plugins.get_func_column(
                    section - len(self.Headers)
                )
            except IndexError:
                # Not enough columns
                return None
    ...
    def _get_column_data(self, func, idx):
        if idx == self.NAME_COL:
            ...
            return self.workspace.plugins.extract_func_column(
                func, idx - len(self.Headers)
            )[0]
...
class QHFunctionTableView(QTableView):
    def __init__(self, parent, workspace, selection_callback=None, plugin=None):
        super(QHFunctionTableView, self).__init__(parent)
        self.workspace = workspace
        self.plugin = plugin
        ...
        self._model = QHFunctionTableModel(self.workspace, [], self.plugin)
    ...
    def load_functions(self):
        if not self.show_alignment_functions:
            self._model.func_list = [
                v for v in self._functions.values() if not v.alignment
            ]
        else:
    ...
    def keyPressEvent(self, key_event):
        ...
        if not text or text not in string.printable or text in string.whitespace:
            # modifier keys
            return super(QHFunctionTableView, self).keyPressEvent(key_event)
...
class QHFunctionTable(QWidget):
    def __init__(self, parent, workspace, selection_callback=None, plugin=None):
        super(QHFunctionTable, self).__init__(parent)
        self.workspace = workspace
        self.plugin = plugin

        self._view = parent  # type: 'FunctionsView'
        self._table_view = None  # type: QHFunctionTableView
    ...
    def function_manager(self, v):
        ...
        if self._table_view is not None:
            ...
            raise ValueError("QHFunctionTableView is uninitialized.")
    ...
    def toggle_show_alignment_functions(self):
        self._table_view.show_alignment_functions = (
            not self._table_view.show_alignment_functions
        )
    ...
    def _init_widgets(self, selection_callback=None):
        ...
        # function table view
        self._table_view = QHFunctionTableView(
            self, self.workspace, selection_callback, self.plugin
        )
